  ...
  float model_ss_ (0.005f); //Amostragem do modelo para cálculo dos keyoints
  float scene_ss_ (0.005f); //Amostragem da cena para cálculo dos keyoints

  float rf_rad_ (0.015f);
  float descr_rad_ (0.04f); //Raio para o cálculo do descritor

  float cg_size_ (0.08f); //Cluster size
...
start = std::clock();                                    // Debug time
  ...
  ///std::string model_directory_;
    if (pcl::console::parse_argument (argc, argv, "--md", model_directory_) != -1)
    {
      std::cout << "model_directory_=" << model_directory_ << std::endl;
  //TODO criar a list de strings usando uma analise da diretoria (dir)

  //model_filenames_.push_back("../models/2.pcd");
 // model_filenames_.push_back("../models/8.pcd");
 // model_filenames_.push_back("../models/9.pcd");
  
 // model_filenames_.push_back("../models/model062.pcd");
 ...
 std::cout << "Time loading Files:  " << diffclock(start, end) << " ms" << endl << endl;
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  printf("Loading %ld models\n", model_filenames_.size());
  for(size_t v=0; v<model_filenames_.size(); ++v)///////////////////////////////////////////////////////////////////////////////
  {

    printf("Analysing for model view %ld\n", v);
...
std::cout << "Time computing clouds resolution:  " << diffclock(start, end) << " ms" << endl << endl;
  ...
  std::cout << "Model resolution:  " << res_model << std::endl;
    ...
    std::cout << "Time computing normals:  " << diffclock(start, end) << " ms" << endl << endl;
    ...
    //
          ...
          pcl::copyPointCloud(*model_normals, keypointIndices1.points, *model_keypoints_normals);
            ...
            std::cout << "Time computing Model Keypoints:  " << diffclock(start, end) << " ms" << endl << endl;
  ...
  */
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////// 
          ...
          descr_est.compute (*model_descriptors);
            ...
            std::cout << "Time computing Model Descriptors:  " << diffclock(start, end) << " ms" << endl << endl;
          ...
          descr_est.compute (*scene_descriptors);
            ...
            std::cout << "Time computing Scene Descriptors:  " << diffclock(start, end) << " ms" << endl << endl;
          //}
  ...
  */
  ...
  //{
    ...
    for (size_t i = 0; i < scene_descriptors->size (); ++i)
    {
      ...
      if(found_neighs == 1 && neigh_sqr_dists[0] < 700.0f) //  add match only if the squared descriptor distance is less than 0.25 (SHOT descriptor distances are between 0 and 1 by design)
      {
        pcl::Correspondence corr (neigh_indices[0], static_cast<int> (i), neigh_sqr_dists[0]);
        model_scene_corrs->push_back (corr);
      }
    ...
    std::cout << "Correspondences found: " << model_scene_corrs->size () << std::endl;
      ...
      std::cout << "Time to find scene-model correspondences:  " << diffclock(start, end) << " ms" << endl << endl;
  //}


  
model_view.push_back(chave());
    ...
    //**************  Using Hough3D
          ...
          clusterer.recognize (model_view.at(v).rototranslations, model_view.at(v).clustered_corrs);
          }
          ...
          //printf("Adding %ld rototranslations")
          gc_clusterer.recognize (model_view[v].rototranslations, model_view[v].clustered_corrs);
            end = std::clock();                               // Debug time
            std::cout << "Time clustering:  " << diffclock(start, end) << " ms" << endl << endl;
          }
    ...
    //  Output results
    //
    std::cout << "Model instances found: " << model_view.at(v).rototranslations.size () << std::endl;
    for (size_t i = 0; i < model_view.at(v).rototranslations.size (); ++i)
    {
      std::cout << "\n    Instance " << i + 1 << ":" << std::endl;
      std::cout << "        Correspondences belonging to this instance: " << model_view.at(v).clustered_corrs[i].size () << std::endl;
