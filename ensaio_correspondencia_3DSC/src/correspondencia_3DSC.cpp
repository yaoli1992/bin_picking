...
float model_ss_ (0.01f); //amostragem do modelo em milimetros
float scene_ss_ (0.01f); //amostragem da cena em milimetros

float norm_rad_ (0.1f); //raio da esfera para o cálculo das normmais

float rf_rad_ (0.05f);
float descr_rad_ (0.04f); //raio da esfera para o cálculo dos descritores

// Resolution of the consensus set used to cluster correspondences together,
	// in metric units. Default is 1.0.
float cg_size_ (0.08f); //tamanho minimo do cluster dos objectos
// Minimum cluster size. Default is 3 (as at least 3 correspondences
	// are needed to compute the 6 DoF pose).
float cg_thresh_ (3000.0f); //

float Lim_dist_ (200000.0f); //Distancia minima entre descritores para que se considere válida a correspondencia
...
parseCommandLine (int argc, char *argv[])
{
  ...
  {
    if (used_algorithm.compare ("Hough") == 0)  //霍夫变化
    {
      use_hough_ = true;
    }
    else if (used_algorithm.compare ("GC") == 0)   //geometry consietency几何一致性
    {
      use_hough_ = false;
    }
    else
...
main (int argc, char *argv[])
{
  ...
  // Recolha dos view points para calculo das normais   //设置视点
...
//
      ...
      //  pcl::copyPointCloud(*scene_normals, keypointIndices2.points, *scene_keypoints_normals);
        ...
        norm_est.setRadiusSearch(norm_rad_);
//norm_est.setKSearch (100);
        norm_est.setInputCloud (scene_keypoints);
        norm_est.compute (*scene_keypoints_normals);
std::cout << scene_keypoints_normals->size() << std::endl;//Debug
        norm_est.setInputCloud (model_keypoints);
        norm_est.setViewPoint(vpx,vpy,vpz);
        norm_est.compute (*model_keypoints_normals);
std::cout << model_keypoints_normals->size() << std::endl;//Debug
...
/*
        ...
        descr_est.setSearchSurface (model_keypoints);
        descr_est.compute (*model_descriptors);

        descr_est.setInputCloud (scene_keypoints);
        descr_est.setInputNormals (scene_keypoints_normals);
        descr_est.setSearchSurface (scene_keypoints);
        descr_est.compute (*scene_descriptors);
*/


  //
  //  Compute Descriptor for keypoints 3DSC
  //
        pcl::search::KdTree<pcl::PointXYZ>::Ptr kdtree(new pcl::search::KdTree<pcl::PointXYZ>);

        pcl::ShapeContext3DEstimation<PointType, NormalType, DescriptorType> descr_est;
...
/*
  //
  //  Compute Descriptor for keypoints Spin
  //
...
//}
  ...
  if (use_hough_)
  {
    ...
    rf_est.setInputNormals (model_keypoints_normals);
    rf_est.setSearchSurface (model_keypoints);
    rf_est.compute (*model_rf);

    rf_est.setInputCloud (scene_keypoints);
    rf_est.setInputNormals (scene_keypoints_normals);
    rf_est.setSearchSurface (scene_keypoints);
...
//std::cout << off_scene_model_keypoints->size() << std::endl; //debug
  ...
  if (show_normals_)
  {
  viewer.addPointCloudNormals<pcl::PointXYZ, pcl::Normal> (scene_keypoints, scene_keypoints_normals, 1, 0.005, "normals");
  viewer.addPointCloudNormals<pcl::PointXYZ, pcl::Normal> (off_scene_model_keypoints, model_keypoints_normals, 1, 0.005, "normal");
  }
...
std::cout << rototranslations.size () << std::endl; //debug
  for (size_t i = 0; i < rototranslations.size (); i++)
  {
...
std::cout << "scene_descriptors size:" << scene_descriptors->size() << std::endl;
  for (size_t i = 0; i < scene_descriptors->size (); i++) //i+=10
  {
    std::string number;
    std::string ValDes;
        ...
        ss1 << i;
          ...
          for(size_t j = 0; j < 125; j++)
          {
            std::ostringstream ss;
                ...
                ss << scene_descriptors->at(i).descriptor[j];
  ...
  std::cout << "model_descriptors size:" << model_descriptors->size() << std::endl;
    for (size_t i = 0; i < model_descriptors->size (); i++) //i+=10
    {
      std::string number;
      std::string ValDes;
          ...
          ss2 << i;
            ...
            for(size_t j = 0; j < 125; j++)
            {
              std::ostringstream ss;
                  ...
                  ss << model_descriptors->at(i).descriptor[j];
